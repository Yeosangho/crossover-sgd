...
parser.add_argument('--lrdecay', default='poly', type=str)
parser.add_argument('--lars_coef', default=0.01, type=float)
parser.add_argument('--baselr', default=3.2, type=float)
...
def make_dataloader(rank, batch_size, world_size):
	...
	train_dataset = torchvision.datasets.CIFAR10(root='./data', train=True, download=False, transform=transforms.Compose([
	...
	train_loader = torch.utils.data.DataLoader(
		train_dataset, batch_size=batch_size, shuffle=False, pin_memory=False, num_workers=2, sampler=train_sampler)
...
def make_validation_dataloader(batch_size):
	...
	val_loader = torch.utils.data.DataLoader(
		datasets.CIFAR10(root='./data', train=False, download=False, transform=transforms.Compose([
			transforms.ToTensor(),
			normalize,
		])),
...
def sgp_comm(param, receive_from, send_to):
	completed_send = dist.isend(tensor=param, dst=send_to)
	
	dist.recv(tensor=param, src=receive_from)
