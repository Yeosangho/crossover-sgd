...
#	##
	...
	##
					if(crossover_flag == True and sync_grad == True):
						crossover(model, model_cp, rank, world_size, model_name, rseed_per_rank, roulettes=roulettes, amp=amp, optimizer=optimizer, sync_grad=sync_grad, amp_flag=amp_flag)		
					
			
					if((crossover_flag == False) and (allreduce == False) and (sync_grad == True)):
						sgp(local_itr, model, model_cp, rank, world_size, epoch, itr, len_loader, amp=amp, optimizer=optimizer, sync_grad=sync_grad, amp_flag=amp_flag)
	##
					if(clip_grad == True):
...
##
	...
	##
				if(crossover_flag == True and sync_grad == False):
					crossover(model, model_cp, rank, world_size, model_name, rseed_per_rank, roulettes=roulettes, amp=amp, optimizer=optimizer, sync_grad=sync_grad, amp_flag=amp_flag, layer_idx=layer_idx)		
	##
				if((crossover_flag == False) and (allreduce == False) and (sync_grad == False)):
					sgp(local_itr, model, model_cp, rank, world_size, epoch, itr, len_loader, amp=amp, optimizer=optimizer, sync_grad=sync_grad, amp_flag=amp_flag, iter_num=iter_num)
				iter_num = iter_num +1
...
#
	...
	train_dataset = torchvision.datasets.CIFAR10(root='/scratch/x2026a02/data', train=True, download=True, transform=transform_train)
	...
	train_loader = torch.utils.data.DataLoader(
		train_dataset, batch_size=batch_size, shuffle=False, pin_memory=True, num_workers=16, sampler=train_sampler)
...
def make_validation_dataloader(batch_size):
	...
	val_set = torchvision.datasets.CIFAR10(root='/scratch/x2026a02/data', train=False, download=True, transform=transform_test)
	val_loader = torch.utils.data.DataLoader(
		val_set,
		batch_size=batch_size, num_workers=16, shuffle=False, pin_memory=True)
...
def sgp(local_itr, model, model_cp, rank, world_size, epoch, itr, max_iter, amp=None, optimizer=None, sync_grad=False, amp_flag=False, iter_num=0):
...
def crossover(model, model_cp, rank, world_size, model_name, rseed_per_rank, roulettes=None, amp=None, optimizer=None, sync_grad=False, amp_flag=False, layer_idx=None):
