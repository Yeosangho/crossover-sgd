...
def main():
	...
	for epoch in range(0, maxepoch):
		...
		for itr,(batch, target) in enumerate(_train_loader, start=0):
			...
			if((rank % proc_per_node) !=  proc_per_node -1 ):
...
#	##
	...
	###
							if grad_clone is None or torch.isnan(grad_clone).any() or torch.isinf(grad_clone).any():
								grad_clone.fill_(0)
	###
							dist.all_reduce(param_clone.data, op=dist.ReduceOp.SUM, group=groups[int(rank/group_size)])
							dist.all_reduce(grad_clone.data, op=dist.ReduceOp.SUM, group=groups[int(rank/group_size)])
	###
...
##
	...
	##
		...
		if(((epoch+1)%val_iter == 0) and (rank % proc_per_node == 0)):
			...
			with open(f'/scratch/x2026a02/{args.tag}_{file_prefix}_{world_size}_{batch_size}_{local_itr}_{rank}_sync_lars_start_at_{sync_lars_start_epoch}_group_num_{group_num}_amp_{amp_flag}_clip_grad_{clip_grad}_baselr_{baselr}_maxepoch_{maxepoch}_lrdecay_{lrdecay}_lars_{lars}_lars_coef_{lars_coef}_chromo_{args.chromosome}_'+'val.csv', '+a') as f:
				print('{ep}, {rank}, '
					'{loss:.4f},'
					'{top1:.3f},'
					'{val}, {elapsed_time}'
...
#
		...
		dist.barrier()
		tensor_flatten3 = flatten_tensors(part_model)
		time.sleep(0.3)
		#sgp_comm(tensor_flatten2, receive_from2, send_to2)
		completed_send3 = dist.isend(tensor=tensor_flatten3, dst=send_peers[2])
		
		dist.recv(tensor=tensor_flatten3, src=recv_peers[2])
		
		completed_send3.wait()
		dist.barrier()
		...
		tensor_unflatten3 = unflatten_tensors(tensor_flatten3, part_model)
		for p, cp, cp2, cp3 in zip(part_model, tensor_unflatten, tensor_unflatten2, tensor_unflatten3):
			p.data.copy_((cp3.data + cp2.data + cp.data +p.data)/(peers_per_itr+1))
...
##
	...
	#	param_clone2 = param.detach().clone()
##
##
	#	sgp_comm(param_clone, receive_from, send_to, iter_num)
	#	sgp_comm(param_clone2, receive_from2, send_to2, iter_num+1)
##
##
	#	param.data.copy_((1./3)*(param.data + param_clone.data + param_clone2.data))
##
	#	
...
def crossover(model, rank, world_size, model_name, rseed_per_rank, roulettes=None, amp=None, optimizer=None, sync_grad=False, amp_flag=False, layer_idx=None, groups=None, group_roots=None, group_size=None, group_num=None):
	...
	#	
	#	tensors.append(tensor_flatten)
	#	send_flag.append(completed_send)
	#	recv_flag.append(completed_recv)
	#	
	#for i,j in zip(send_flag, recv_flag):
	#	i.wait()
	#	j.wait()
#
	#for i in range(len(layer_idx)):
