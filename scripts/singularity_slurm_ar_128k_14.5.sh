...
#SBATCH -e %x_%j.err
echo $SLURM_JOBID
cat singularity_slurm_ar_128k_14.5.sh
...
export PROCESS_NUM = 8
mpirun -n $PROCESS_NUM  -npernode $PROC_PER_NODE singularity exec --nv /scratch/x2026a02/200909_pytorch.sif /opt/anaconda3/envs/pytorch4/bin/python /scratch/x2026a02/imagenet_test.py --batch_size 256 --world_size $PROCESS_NUM --crossover False --tag $SLURM_JOBID --lars True --allreduce True --amp True --lrdecay poly --lars_coef 0.0025 --baselr 14.5 --warmup_epoch 36 --local_itr 64 --clip_grad True --sync_grad True --sync_lars_start_epoch 200 --sync_lars_group_size $PROCESS_NUM --chromosome fine --maxepoch 90 --manual_seed 776 --gpu_per_node 2 --proc_per_node $PROC_PER_NODE
